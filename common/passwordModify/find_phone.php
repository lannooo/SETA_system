...
if(!$stmt->fetch()){
    ...
    $stmt->close();
    if ($group == "teacher") {
        $sql = "SELECT phone FROM teacher WHERE tid=" . $id;
        ...
        if ($result == false) {
            ...
            echo('{"code":200, "message": "合法用户名", "phone":' . $row['phone'] . '}');
        }
    } else if ($group == "student") {
        ...
        if ($v) {
            $sql = "SELECT phone FROM student_info WHERE sid=" . $id;
        ...
        $sql = "SELECT phone FROM ta_info WHERE taid=" . $id;
